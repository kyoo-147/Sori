...
export const VoiceEditScreen: React.FC<Props> = ({ runtimeSource = 'unavailable', runtimeClient }) => {
  ...
  const detectSelection = () => {
    ...
    setSelection({ target_identity: 'browser:selection', text }); setState('blocked'); setDetail('Browser preview selection recorded locally. Native focused-app target identity is UNAVAILABLE, so diff generation and replacement stay disabled.');
  };
  ...
  const runPreview = async () => {
    ...
    if (selection.target_identity === 'browser:selection') { setState('blocked'); setDetail('Unavailable: browser preview selection cannot be sent as native focused-app evidence.'); return; }
  ...
  const apply = async () => {
    if (!selection || !preview || selection.target_identity === 'browser:selection') { setState('blocked'); setDetail('Unavailable: browser preview cannot inject into a focused application.'); return; }
  ...
  return <div className="mx-auto max-w-[1180px] space-y-6 p-4 text-[#1C1B19]">
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(340px,0.85fr)_minmax(0,1.35fr)]">
      ...
      <section className="sori-pane p-5"><div className="flex items-center justify-between"><div><h2 className="sori-section-heading">Diff preview</h2><p className="sori-meta-text mt-1">Approval is required; target is revalidated before injection.</p></div><button type="button" onClick={runPreview} disabled={!selection || selection.target_identity === 'browser:selection' || state === 'capturing'} className="sori-tactile-btn rounded-[10px] px-3 py-2 text-xs"><Eye className="mr-1 inline h-3.5 w-3.5" />{selection?.target_identity === 'browser:selection' ? 'Preview unavailable' : 'Generate diff'}</button></div>{state === 'capturing' ? <div className="flex min-h-[180px] items-center justify-center"><LoaderCircle className="h-6 w-6 animate-spin text-[#6E7A80]" /></div> : preview ? <pre className="mt-4 overflow-auto rounded-xl bg-[#FFFDF9] p-4 text-xs leading-6">{preview.diff}</pre> : <div className="mt-4 rounded-xl border border-dashed border-[#DED9D1] p-8 text-center text-xs text-[#98928A]">No generated diff.</div>}<div className="mt-4 flex items-start gap-2 rounded-[12px] bg-[#F2EEE8] p-3 text-[11px] text-[#68635D]"><AlertTriangle className="h-4 w-4 shrink-0 text-[#9A7442]" />{state === 'applied' ? detail : 'Browser selection detection is not native focused-app evidence.'}</div><button type="button" onClick={apply} disabled={!preview || selection?.target_identity === 'browser:selection' || state === 'capturing' || state === 'applied'} className="mt-4 rounded-[10px] bg-[#1C1B19] px-4 py-2 text-xs font-medium text-[#FFFDF9]"><Check className="mr-1 inline h-3.5 w-3.5" />Approve &amp; replace</button></section>
    </div>
  </div>;
};
