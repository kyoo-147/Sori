...
export function mapModels(value: unknown): ModelRecord[] {
  ...
  return payload.models.map(({ manifest, status }) => {
    ...
    return { id, name: manifest.display_name, provider, location: 'local', qualityTier: 'standard', recommended: false, available: status.installed, installed: status.installed, loaded: status.loaded, warm: status.warm, unavailableReason: status.error ?? (status.installed ? null : 'Executable and model assets are not both verified'), source: manifest.source ?? null, sha256: manifest.sha256 ?? null, license: manifest.license?.name ?? null };
  });
}
export class HttpIpcTransport implements IpcTransport { readonly source = 'backend' as const; constructor(private readonly url = endpoint, private readonly fetchImpl: typeof fetch = globalThis.fetch.bind(globalThis)) {} async request(operation: IpcOperation, params?: Record<string, unknown>): Promise<unknown> { const response = await this.fetchImpl(this.url, { method: 'POST', headers: { 'content-type': 'application/json', accept: 'application/json' }, body: JSON.stringify(requestShape(operation, params)), signal: AbortSignal.timeout(2_000) }); if (!response.ok) throw new Error(`IPC request failed (${response.status})`); return response.json(); } }
...
export class DesktopIpcTransport implements IpcTransport { private activeSource: Exclude<RuntimeSource, 'unavailable'> = 'native'; constructor(private readonly native: IpcTransport = new NativeIpcTransport(), private readonly http: IpcTransport = new HttpIpcTransport()) {} get source() { return this.activeSource; } async request(operation: IpcOperation, params?: Record<string, unknown>): Promise<unknown> { if (this.native.isAvailable?.() === false) { const value = await this.http.request(operation, params); this.activeSource = 'backend'; return value; } try { const value = await this.native.request(operation, params); this.activeSource = 'native'; return value; } catch (nativeError) { try { const value = await this.http.request(operation, params); this.activeSource = 'backend'; return value; } catch { throw nativeError; } } } }
