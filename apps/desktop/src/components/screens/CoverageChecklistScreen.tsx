...
export const CoverageChecklistScreen: React.FC<Props> = ({ checks = [], runtimeSource = 'unavailable', runtimeStatus, runtimeError, onRefresh }) => {
  ...
  return <div className="sori-screen sori-page-layout space-y-6">
    ...
    </div><button className="sori-tactile-btn rounded-xl px-4 py-2.5 text-sm" onClick={refresh} disabled={loading}><RefreshCw className={`mr-1 inline h-4 w-4 ${loading ? 'animate-spin' : ''}`} />Run Doctor Check</button><p className="sori-meta-text mt-1 w-full">Source: {runtimeSource} · Activity: {runtimeStatus?.activity ?? 'unknown'}{runtimeError ? ` · ${runtimeError}` : ''}</p>
</header>
