...
export const DesktopSidebar: React.FC<DesktopSidebarProps> = ({ activeScreen, setActiveScreen, openSettingsModal, isOpen = true, onClose, collapsed = false }) => {
  ...
  return <aside id="sori-navigation" className={`${isOpen && !collapsed ? 'flex' : 'hidden'} sori-shell__sidebar md:flex max-md:fixed max-md:top-10 max-md:inset-y-0 max-md:left-0 max-md:z-40`} data-open={isOpen && !collapsed} data-collapsed={collapsed}>
