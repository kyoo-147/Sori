    ...
    <main className="mx-auto min-w-0 max-w-6xl space-y-6 p-4 md:p-8" aria-busy={running}>
      ...
      <div className="grid min-w-0 gap-5 lg:grid-cols-[0.9fr_1.1fr]">
        <section className="sori-pane min-w-0 space-y-5 p-5" aria-labelledby="benchmark-execution-heading">
          ...
          <p className="text-xs text-[var(--sori-text-tertiary)]">No fixture or ready provider means unavailable, never fake success. Timeout and cancel results are not persisted as successful benchmarks.</p>
        </section>
        <section className="sori-pane min-w-0 space-y-4 p-5" aria-labelledby="persisted-results-heading">
          <div className="flex items-center justify-between border-b border-[var(--sori-border-soft)] pb-3"><div><h2 id="persisted-results-heading" className="sori-section-heading">Persisted results</h2><p className="sori-meta-text">SQLite-backed provider results</p></div><button className="sori-tactile-btn rounded-lg px-3 py-2 text-xs" onClick={exportResults}><Download className="mr-1 inline h-4 w-4" aria-hidden="true" />Export</button></div>
