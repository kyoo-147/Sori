...
const evidencePath = join(artifactDir, `${runId}.json`);
const db = join(artifactDir, `${runId}.db`);
const owner = join(artifactDir, `${runId}.daemon-owner.json`);
...
function startDaemon(database: string): ChildProcess {
  const child = spawn(binaryPath('sorid'), [], { stdio: ['ignore', 'pipe', 'pipe'], shell: false, env: { ...process.env, SORI_IPC_URL: endpoint.toString(), SORI_IPC_ADDR: `${endpoint.hostname}:${endpoint.port || '80'}`, SORI_DATABASE_PATH: database, SORI_DB_PATH: database, SORI_DAEMON_OWNER_PATH: owner, SORI_E2E: '1' } });
...
function cleanupOwnedRunFiles(): void {
  // Only remove paths generated by this invocation; ambiguous legacy owners remain untouched.
  for (const path of [owner, db, `${db}-wal`, `${db}-shm`, `${owner}.tmp`]) rmSync(path, { force: true });
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    try { await stopDaemon(daemon); }
    finally {
      try { writeFileSync(evidencePath, JSON.stringify({ endpoint: endpoint.toString(), database: db, owner, observations }, null, 2)); }
      finally { cleanupOwnedRunFiles(); }
    }
  }
}

main().catch((error) => { try { writeFileSync(evidencePath, JSON.stringify({ endpoint: endpoint.toString(), database: db, owner, observations, error: error instanceof Error ? error.stack ?? error.message : String(error) }, null, 2)); } finally { cleanupOwnedRunFiles(); } console.error(`FAIL: ${error instanceof Error ? error.stack ?? error.message : String(error)}`); process.exitCode = 1; });
