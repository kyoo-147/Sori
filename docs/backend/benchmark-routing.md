...
Every successful run also records a framed input fingerprint covering the complete audio chunk sequence, every chunk format, and reference presence/content, plus optional model/runtime SHA-256 values supplied by the provider environment and explicit unavailable reasons for missing RAM/VRAM/fallback telemetry. Recommendations are only computed among runs with the same input fingerprint; mixed-input history is never compared.
