...
export const ModelManagerScreen: React.FC<Props> = ({ runtimeClient, onActiveModelChanged }) => {
  ...
  const lifecycle = async (model: ModelRecord, operation: 'load' | 'warm' | 'unload') => {
    setSaving(true); setError(null);
    const result = await (operation === 'load' ? runtimeClient.loadModel(model.id) : operation === 'warm' ? runtimeClient.warmModel(model.id) : runtimeClient.unloadModel(model.id));
    if (result.error) setError(result.error); else await load();
    setSaving(false);
  };
