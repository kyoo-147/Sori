...
export type EventKind = 'AudioStarted' | 'AudioChunkCaptured' | 'AudioStopped' | 'AudioError' | 'HotkeyPressed' | 'HotkeyReleased' | 'HotkeyCancelled' | 'VadSpeechStarted' | 'VadSpeechEnded' | 'AsrSelected' | 'TranscriptPartial' | 'TranscriptFinal' | 'IntentDetected' | 'ActionBefore' | 'ActionAfter' | 'PermissionRequested' | 'ModelFallback' | 'DictationCancelled' | 'ExtensionInvoked' | 'TtsStarted' | 'TtsFinished' | 'SpeakerVerified' | 'SpeakerRejected' | 'DaemonReady' | 'DaemonPaused' | 'DaemonError' | 'DaemonShuttingDown' | 'CapabilityAvailable' | 'CapabilityUnavailable' | 'ResourceChanged' | 'SettingChanged' | 'ModelChanged' | 'HistoryChanged';
...
export type IpcRequest =
  | 'Status' | 'Doctor' | 'AudioReadiness' | 'ConfigSummary' | 'Models' | 'DictationStart' | 'DictationStop' | 'DictationCancel' | 'PurgeHistory' | 'DeleteHistory' | 'Pause' | 'Resume' | 'ExtensionsList'
  | { RecentEvents: { limit: number } } | { ResourceGet: { resource: string } } | { ResourceSet: { resource: string; value: unknown } } | { ResourceDelete: { resource: string } } | { RecentHistory: { limit: number } } | { DeleteHistory: { id: string } } | { SettingGet: { key: string } } | { SettingDelete: { key: string } } | { SetConfig: { key: string; value: unknown } } | { Dictation: { model: string; audio: AudioChunk[] } } | { DictationAudio: { model: string; audio: AudioChunk[]; injection_strategy?: 'DirectInput' | 'ClipboardPaste' } }
...
export type AudioReadinessState = 'Unavailable' | 'PermissionRequired' | 'DeviceUnavailable' | 'Ready';
export interface AudioReadinessResponse { state: AudioReadinessState; configured: boolean; detail: string; signal: 'UNVERIFIED' | string; }
...
export interface SettingResponse { key: string; value: unknown | null; }
export interface IpcResponseMap { Status: StatusResponse; Doctor: DoctorResponse; AudioReadiness: AudioReadinessResponse; ConfigSummary: ConfigSummaryResponse; Setting: SettingResponse; Models: ModelsResponse; ModelStatus: { provider: string; status: ModelRuntimeStatus }; RecentEvents: RecentEventsResponse; RecentHistory: RecentHistoryResponse; Resource: ResourceResponse; Control: ControlResponse; Transcript: TranscriptResponse; VoiceEdit: VoiceEditResponse; Extensions: { extensions: ExtensionRecord[] }; Benchmark: unknown; Error: { code: string; detail: string }; }
export type IpcResponse = { [K in keyof IpcResponseMap]: { [P in K]: IpcResponseMap[K] } }[keyof IpcResponseMap];
export type IpcOperation = 'status' | 'doctor' | 'audio_readiness' | 'config_summary' | 'setting_get' | 'setting_delete' | 'models' | 'model_status' | 'model_load' | 'model_warm' | 'model_unload' | 'model_install' | 'model_remove' | 'recent_events' | 'recent_history' | 'resource_get' | 'resource_set' | 'resource_delete' | 'purge_history' | 'set_config' | 'dictation_start' | 'dictation_stop' | 'dictation_cancel' | 'dictation_audio' | 'voice_edit' | 'pause' | 'resume' | 'extensions_list' | 'extension_install' | 'extension_enable' | 'extension_disable' | 'extension_uninstall' | 'extension_invoke' | 'run_benchmark' | 'cancel_benchmark' | 'recent_benchmarks' | 'apply_benchmark_recommendation' | 'delete_history';
export function requestShape(operation: IpcOperation, params: Record<string, unknown> = {}): IpcRequest {
  switch (operation) {
    case 'status': return 'Status'; case 'doctor': return 'Doctor'; case 'audio_readiness': return 'AudioReadiness'; case 'config_summary': return 'ConfigSummary'; case 'setting_get': return { SettingGet: { key: String(params.key ?? '') } }; case 'setting_delete': return { SettingDelete: { key: String(params.key ?? '') } }; case 'models': return 'Models';
    ...
    case 'dictation_start': return 'DictationStart'; case 'dictation_stop': return 'DictationStop'; case 'dictation_cancel': return 'DictationCancel'; case 'dictation_audio': { const strategy = params.injection_strategy; return { DictationAudio: { model: String(params.model ?? ''), audio: (params.audio ?? []) as AudioChunk[], ...(strategy === 'DirectInput' || strategy === 'ClipboardPaste' ? { injection_strategy: strategy } : {}) } }; }
...
export function isIpcResponse(value: unknown): value is IpcResponse { return !!value && typeof value === 'object' && ['Status', 'Doctor', 'AudioReadiness', 'ConfigSummary', 'Setting', 'Models', 'ModelStatus', 'RecentEvents', 'RecentHistory', 'Resource', 'Control', 'Transcript', 'VoiceEdit', 'Extensions', 'Benchmark', 'Error'].some((variant) => variant in (value as object)); }
