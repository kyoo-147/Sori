...
export const StudioSettingsScreen: React.FC<StudioSettingsScreenProps> = ({ settings, setSettings, runtimeClient, onClose, closeRef, compact = false }) => {
  ...
  const moveTheme = (offset: number) => {
    const next = (themeIndex + offset + soriThemes.length) % soriThemes.length;
  ...
  const renderContent = () => {
    ...
    if (activeTab === 'General') return <section className="space-y-4"><p className="settings-copy">Choose a workspace palette.</p><div className="settings-group"><span className="settings-label" id="theme-label">Workspace palette</span><div className="theme-picker" role="radiogroup" aria-labelledby="theme-label" onKeyDown={(event) => { if (event.key === 'ArrowRight' || event.key === 'ArrowDown') { event.preventDefault(); moveTheme(1); } else if (event.key === 'ArrowLeft' || event.key === 'ArrowUp') { event.preventDefault(); moveTheme(-1); } else if (event.key === 'Home') { event.preventDefault(); setSettings((current) => ({ ...current, theme: soriThemes[0] })); } else if (event.key === 'End') { event.preventDefault(); setSettings((current) => ({ ...current, theme: soriThemes[soriThemes.length - 1] })); } }}>{soriThemes.map((id) => <button key={id} type="button" role="radio" aria-checked={settings.theme === id} tabIndex={settings.theme === id ? 0 : -1} className={'theme-picker__option theme-picker__option--' + id + (settings.theme === id ? ' is-selected' : '')} onClick={() => setSettings((current) => ({ ...current, theme: id }))}><span className="theme-picker__swatch" style={{ backgroundColor: themePalettes[id].primary }} aria-hidden="true" />{themeLabels[id]}</button>)}</div></div><div className="settings-group"><label htmlFor="language-region" className="settings-label">Language &amp; region</label><select id="language-region" disabled aria-label="Language and region preview" className="settings-input"><option>English (US)</option><option>Vietnamese (Tiếng Việt)</option><option>Bilingual Auto-Detect (EN / VI)</option></select><p className="settings-help">Additional languages are not yet available.</p></div></section>;
