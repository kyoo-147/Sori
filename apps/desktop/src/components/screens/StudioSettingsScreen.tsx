...
export const StudioSettingsScreen: React.FC<StudioSettingsScreenProps> = ({ settings, setSettings, runtimeClient, onClose, closeRef, compact = false }) => {
  ...
  const renderContent = () => {
    ...
    if (activeTab === 'General') return <section className="space-y-4"><p className="settings-copy">Choose a workspace palette. Language selection is not available yet.</p><div className="settings-group"><span className="settings-label" id="theme-label">Workspace palette</span><div className="theme-picker" role="radiogroup" aria-labelledby="theme-label">{soriThemes.map((id) => <button key={id} type="button" role="radio" aria-checked={settings.theme === id} className={'theme-picker__option theme-picker__option--' + id + (settings.theme === id ? ' is-selected' : '')} onClick={() => setSettings((current) => ({ ...current, theme: id }))}><span className="theme-picker__swatch" style={{ backgroundColor: themePalettes[id].primary }} aria-hidden="true" />{themeLabels[id]}</button>)}</div><p className="settings-help">Used across Sori and saved on this device.</p></div><div className="settings-group"><label htmlFor="language-region" className="settings-label">Language &amp; region</label><select id="language-region" disabled aria-label="Language and region preview" className="settings-input"><option>English (US)</option><option>Vietnamese (Tiếng Việt)</option><option>Bilingual Auto-Detect (EN / VI)</option></select><p className="settings-help">Additional languages are not yet available.</p></div></section>;
