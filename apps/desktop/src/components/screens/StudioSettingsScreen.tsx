...
export const StudioSettingsScreen: React.FC<StudioSettingsScreenProps> = ({ settings, setSettings, runtimeClient, onClose, closeRef, compact = false }) => {
  ...
  const renderContent = () => {
    ...
    if (activeTab === 'General') return <section className="space-y-4"><p className="settings-copy">Set the language and appearance for your workspace.</p><div className="settings-group"><span className="settings-label" id="theme-label">Workspace palette</span><div className="theme-picker" role="radiogroup" aria-labelledby="theme-label">{([['clear', 'Clear'], ['brown', 'Brown'], ['green', 'Green'], ['blue', 'Blue']] as const).map(([id, label]) => <button key={id} type="button" role="radio" aria-checked={settings.theme === id} className={'theme-picker__option theme-picker__option--' + id + (settings.theme === id ? ' is-selected' : '')} onClick={() => setSettings((current) => ({ ...current, theme: id }))}><span className="theme-picker__swatch" aria-hidden="true" />{label}</button>)}</div><p className="settings-help">Used across Sori and saved on this device.</p></div><div className="settings-group"><label htmlFor="language-region" className="settings-label">Language &amp; region</label><select id="language-region" disabled aria-label="Language and region preview" className="settings-input"><option>English (US)</option><option>Vietnamese (Tiếng Việt)</option><option>Bilingual Auto-Detect (EN / VI)</option></select><p className="settings-help">Additional languages are not yet available.</p></div></section>;
  ...
  return <div className={compact ? 'settings-panel settings-panel--modal' : 'settings-panel'}>
    <header className="settings-header"><div><h1 className="settings-title">Settings</h1><p className="settings-subtitle">Personalize your dictation workspace.</p></div>{onClose && <button type="button" onClick={onClose} ref={closeRef} className="settings-close" aria-label="Close settings"><X aria-hidden="true" /></button>}</header>
