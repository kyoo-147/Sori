...
export const StudioSettingsScreen: React.FC<StudioSettingsScreenProps> = ({ settings, setSettings, runtimeClient, onClose, closeRef, compact = false }) => {
  ...
  const selectTheme = (index: number, close = true) => {
    ...
    if (close) setThemePickerOpen(false);
    themeOptionRefs.current[next]?.focus();
  };
  useEffect(() => {
    if (!themePickerOpen) return;
    const onPointerDown = (event: PointerEvent) => {
      if (themePickerRef.current && !themePickerRef.current.contains(event.target as Node)) setThemePickerOpen(false);
    };
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') { event.preventDefault(); setThemePickerOpen(false); }
    };
