...
try {
  ...
  if ($DeterministicProviderText) { Pass "deterministic SAPI WAV fixture produced explicit test transcript: $($response.Transcript.text)"; $artifact.steps.Add('SAPI WAV traversed canonical audio/VAD/provider seam and actual Windows SendInput path; ASR remains UNVERIFIED') } else { Pass "real Whisper fixture produced transcript: $($response.Transcript.text)"; $artifact.steps.Add('real fixture audio traversed canonical provider and actual Windows SendInput path') }
