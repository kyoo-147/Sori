...
function Get-OwnedDaemon([string]$ExpectedPath, $Listener) {
  ...
  if (-not $process) { Fail 'leased daemon process is not running' }
  return $process
}
...
try {
  ...
  $daemon = Get-OwnedDaemon $daemonPath $listener
  $artifact.daemon.pid = $daemon.Id
  ...
  if ($artifact.preflight.audio_readiness.AudioReadiness.state -ne 'Ready') { Fail "microphone readiness is not Ready: $($artifact.preflight.audio_readiness.AudioReadiness.detail)" }
  $modelRecord = @($artifact.preflight.models.Models.models | Where-Object { $_.manifest.id -eq $Model } | Select-Object -First 1)
  if (-not $modelRecord -or -not $modelRecord[0].status.installed) { Fail "configured model is not installed/discovered: $Model" }
  $artifact.preflight.model = $modelRecord[0]
...
'@ -OutputAssembly $targetExe -OutputType WindowsApplication
  ...
  if ($ExpectedText -and $artifact.transcript -notlike "*$ExpectedText*") { Fail 'transcript did not contain ExpectedText' }
  ...
  $oldDaemonPid = $daemon.Id
  if ($app -and -not $app.HasExited) { Stop-Process -Id $app.Id -Force -ErrorAction SilentlyContinue }
  Stop-Process -Id $oldDaemonPid -Force
  Start-Sleep -Seconds 2; $app = Start-Process -FilePath $appPath -WorkingDirectory (Split-Path -Parent $appPath) -PassThru
  ...
  if (-not $restartStatus.Status.running) { Fail 'daemon did not recover after owned restart' }
  ...
  $artifact.restart = [ordered]@{ status = 'VERIFIED'; restored_history_id = [string]$entry.id; status_response = $restartStatus }
}
