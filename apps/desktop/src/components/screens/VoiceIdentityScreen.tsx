...
export const VoiceIdentityScreen: React.FC<Props> = ({ voiceProfile, setVoiceProfile, history, setHistory, runtimeClient }) => {
  ...
  return <div className="mx-auto max-w-5xl space-y-6 p-4 md:p-8">
    ...
    {confirm && <div className="sori-overlay fixed inset-0 z-50 flex items-center justify-center p-4"><div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="delete-history-title" aria-describedby="delete-history-description" className="w-full max-w-md rounded-2xl bg-[#FFFDF9] p-6"><h2 id="delete-history-title" className="sori-section-heading">Delete local history?</h2><p id="delete-history-description" className="sori-body-text mt-2">This permanently purges persisted SQLite history. Type DELETE to confirm.</p><input data-dialog-initial-focus className="sori-control mt-4 w-full rounded-lg p-2 font-mono" value={typed} onChange={(e) => setTyped(e.target.value)} placeholder="DELETE" aria-label="Type DELETE to confirm" /><div className="mt-5 flex justify-end gap-2"><button type="button" className="px-3 py-2 text-sm" onClick={() => { setConfirm(false); setTyped(''); }}>Cancel</button><button type="button" disabled={typed !== 'DELETE' || busy} className="rounded-lg bg-[#A75850] px-3 py-2 text-sm text-white disabled:opacity-40" onClick={deleteData}>{busy ? 'Deleting…' : 'Delete permanently'}</button></div></div></div>}</div>;
};
