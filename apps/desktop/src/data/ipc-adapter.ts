...
export function createIpcRepositories(transport: IpcTransport): SoriRepositories {
  ...
  const repositories: any = {
    ...
    models: { list: () => resource<ModelRecord[]>('models'), select: async (id: string) => { const saved = await saveResource('route', { modelId: id }); return saved.status === 'error' ? saved as unknown as DataState<{ modelId: string }> : { status: 'ready', data: { modelId: id }, error: null, source: 'ipc' }; } },
