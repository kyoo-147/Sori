...
export const VoiceIdentityScreen: React.FC<Props> = ({ voiceProfile, setVoiceProfile, history, setHistory, runtimeClient }) => {
  ...
  const [policy, setPolicy] = useState(voiceProfile.guestPolicy);
  ...
  return <div className="mx-auto max-w-5xl space-y-6 p-4 md:p-8">
    ...
    </section><section className="sori-pane space-y-4 p-5"><h2 className="sori-section-heading">Voice lock &amp; access</h2><div className="flex items-center gap-3 rounded-xl border border-[#E5E0D9] p-3"><Fingerprint className="h-5 w-5 text-[#6E7A80]" /><div className="flex-1"><div className="text-sm font-medium">Voice Lock (Biometric)</div><div className="sori-meta-text">Hardware capture and biometric verification are UNVERIFIED.</div></div><span className="rounded-full bg-[#FFF7E6] px-2 py-1 text-xs text-[#9A7442]">Unavailable</span></div><label className="block text-sm font-medium">Require owner verification<select className="sori-control mt-2 w-full rounded-lg p-2" value={policy} onChange={(e) => { const next = e.target.value as VoiceProfile['guestPolicy']; setPolicy(next); setVoiceProfile((p) => ({ ...p, guestPolicy: next })); }}><option value="off">Off — allow all speakers</option><option value="guest_dictation_only">Sensitive actions only</option><option value="strict_owner_only">Strict owner only</option></select></label><div className="rounded-lg bg-[#FFF7E6] p-3 text-xs text-[#6B552C]">Voice enrollment and verification are <b>UNVERIFIED</b>; this UI makes no biometric capability claim.</div></section></div>
