...
import { DEFAULT_IPC_URL, binaryPath, parseEndpoint, requireEndpointFree } from '../scripts/e2e-desktop-backend.js';
...
describe('desktop backend E2E helpers', () => {
  ...
  it('allows an unowned endpoint for an isolated run', async () => {
    await expect(requireEndpointFree(new URL('http://127.0.0.1:1/ipc'))).resolves.toBeUndefined();
  });
});
