...
export const StudioSettingsScreen: React.FC<StudioSettingsScreenProps> = ({ settings, setSettings, runtimeClient, onClose, closeRef, compact = false }) => {
  ...
  const renderContent = () => {
    if (activeTab === 'Microphone') return <section className="space-y-4" aria-labelledby="settings-section-title"><p className="settings-copy">Check microphone access and availability for dictation.</p><div className={`settings-status ${micReadiness === 'ready' ? 'settings-status--ready' : 'settings-status--attention'}`} role="status"><span className="settings-status__dot" />{micCheck}</div>{micTestMsg && <div className="settings-note" role="status">{micTestMsg}</div>}<button type="button" onClick={() => void handleTestMic()} disabled={micReadiness === 'checking'} className="settings-button settings-button--primary">{micReadiness === 'checking' ? 'Checking…' : 'Check microphone'}</button><p className="settings-help">Checks available software status only; it does not record audio.</p></section>;
    ...
    if (activeTab === 'General') return <section className="space-y-4"><p className="settings-copy">Language and appearance preferences.</p><div className="settings-group"><span className="settings-label" id="theme-label">Workspace palette</span><div className="theme-picker" role="radiogroup" aria-labelledby="theme-label">{([['clear', 'Clear'], ['brown', 'Brown'], ['green', 'Green'], ['blue', 'Blue']] as const).map(([id, label]) => <button key={id} type="button" role="radio" aria-checked={settings.theme === id} className={'theme-picker__option theme-picker__option--' + id + (settings.theme === id ? ' is-selected' : '')} onClick={() => setSettings((current) => ({ ...current, theme: id }))}><span className="theme-picker__swatch" aria-hidden="true" />{label}</button>)}</div><p className="settings-help">Applied across shared surfaces and saved on this device.</p></div><div className="settings-group"><label htmlFor="language-region" className="settings-label">Language &amp; region</label><select id="language-region" disabled aria-label="Language and region preview" className="settings-input"><option>English (US)</option><option>Vietnamese (Tiếng Việt)</option><option>Bilingual Auto-Detect (EN / VI)</option></select><p className="settings-help">More languages are not yet available.</p></div></section>;
  ...
  return <div className={compact ? 'settings-panel settings-panel--modal' : 'settings-panel'}>
    <header className="settings-header"><div><h1 className="settings-title">Settings</h1><p className="settings-subtitle">Tune Sori for your workflow.</p></div>{onClose && <button type="button" onClick={onClose} ref={closeRef} className="settings-close" aria-label="Close settings"><X aria-hidden="true" /></button>}</header>
    <div className="settings-layout">
      ...
      <main className="settings-content" aria-labelledby="settings-section-title"><div className="settings-content__heading"><div><h2 id="settings-section-title">{activeTab}</h2><span className="settings-content__meta">Saved locally</span></div>{activeTab === 'Microphone' && <span className="settings-chip"><CheckCircle2 aria-hidden="true" /> Local</span>}</div>{configLoading && <div className="settings-note" role="status">Loading canonical settings...</div>}{configError && <div className="settings-status settings-status--error" role="alert">Unable to load settings: {configError}</div>}{renderContent()}{configMsg && <p className="settings-feedback" role="status">{configMsg}</p>}</main>
    </div>
  </div>;
};
